...
const AgentChat: React.FC = () => {
	...
	const sendMessage = () => {
		...
		setTimeout(
			() =>
				setMessages((msgs) => [...msgs, { from: 'catbot', text: 'Received: ' + input }]),
			800
		);
	};
	...
	return (
		<div className="p-6 max-w-xl mx-auto">
			<div className="flex justify-end mb-2">
				<a
					href="/chat"
					className="text-xs text-blue-600 hover:underline bg-blue-50 rounded px-2 py-1"
					...
					<div key={i} className={msg.from === 'user' ? 'text-right' : 'text-left'}>
						<span
							className={`inline-block px-3 py-2 rounded ${
								msg.from === 'user' ? 'bg-blue-100 text-blue-900' : 'bg-gray-200 text-gray-800'
							}`}
						>
							{msg.text}
						</span>
